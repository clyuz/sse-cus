...
func TestCusClientSubscribe(t *testing.T) {

	c := NewClient("http://127.0.0.1/api/v1/events?stream=%22t%22&siteId=14209903554565")

	//events := make(chan *Event)
	err :=c.Subscribe("", func(msg *Event) {
		if msg.Data != nil {
			fmt.Println("per",string(msg.Data))
			//events <- msg
			return
		}
	})
